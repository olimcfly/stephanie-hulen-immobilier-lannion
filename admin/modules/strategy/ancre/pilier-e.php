...
/**
 * ══════════════════════════════════════════════════════════════
 * MÉTHODE ANCRE — Pilier E : Expansion continue
 * /admin/modules/strategy/ancre/pilier-e.php
 * ══════════════════════════════════════════════════════════════
 */
...
$steps = [
    [
        'key'         => 'facebook_ads_vendeurs',
        'num'         => 1,
        'titre'       => 'Lancer une campagne Facebook Ads vendeurs',
        'desc'        => 'Campagne "Vendeurs" ciblée : 35-65 ans, propriétaires, zone géo 10km. Format carrousel ou vidéo courte. Objectif : trafic vers page estimation. Budget de départ : 5-10€/jour. Testez 2 visuels différents.',
        'duree'       => '2h lancement',
        'difficulte'  => 'Moyen',
        'module_lien' => ADMIN_URL . '/dashboard.php?page=facebook-ads',
        'module_nom'  => 'Facebook Ads',
        'module_icon' => 'fa-ad',
        'ressources'  => ['Templates visuels vendeurs','Textes d\'annonces éprouvés','Ciblage audience propriétaires'],
        'tips'        => 'Un visuel "avant/après" (maison ordinaire → vendue rapidement) performe généralement 2x mieux que les visuels génériques.',
    ],
    [
        'key'         => 'pixel_retargeting',
        'num'         => 2,
        'titre'       => 'Installer le pixel et configurer le retargeting',
        'desc'        => 'Le pixel Meta sur votre site permet de recibler les visiteurs de votre page estimation qui n\'ont pas soumis le formulaire. Ces prospects "chauds" convertissent à 5-8x le taux normal pour un coût bien inférieur.',
        'duree'       => '1h',
        'difficulte'  => 'Moyen',
        'module_lien' => ADMIN_URL . '/dashboard.php?page=facebook-ads',
        'module_nom'  => 'Pixel & Retargeting',
        'module_icon' => 'fa-crosshairs',
        'ressources'  => ['Guide installation pixel','Événements à tracker','Campagne retargeting type'],
        'tips'        => 'Créez un "Custom Audience" des visiteurs de la page estimation des 30 derniers jours — c\'est votre audience la plus précieuse.',
    ],
    [
        'key'         => 'audiences_personnalisees',
        'num'         => 3,
        'titre'       => 'Créer 3 audiences personnalisées et Lookalike',
        'desc'        => 'Audience 1 : Visiteurs site web (30j). Audience 2 : Leads CRM importés. Audience 3 : Lookalike basé sur vos meilleurs clients. Les Lookalike audiences réduisent le coût par lead de 40 à 60%.',
        'duree'       => '45 min',
        'difficulte'  => 'Moyen',
        'module_lien' => ADMIN_URL . '/dashboard.php?page=facebook-ads',
        'module_nom'  => 'Audiences',
        'module_icon' => 'fa-users',
        'ressources'  => ['Guide audiences Meta','Export CRM pour Lookalike','Stratégie d\'audiences avancées'],
        'tips'        => 'L\'audience Lookalike 1% est généralement la plus performante — elle trouve vos "sosies" les plus proches.',
    ],
    [
        'key'         => 'programme_partenaires',
        'num'         => 4,
        'titre'       => 'Mettre en place un programme de partenaires',
        'desc'        => 'Identifiez 10 partenaires locaux (notaires, banques, artisans, déménageurs) et créez une relation formelle d\'apport d\'affaires. Chaque partenaire peut vous envoyer 2-5 mandats par an sans effort.',
        'duree'       => '3h setup',
        'difficulte'  => 'Moyen',
        'module_lien' => ADMIN_URL . '/dashboard.php?page=courtiers-partenaires',
        'module_nom'  => 'CRM Partenaires',
        'module_icon' => 'fa-handshake',
        'ressources'  => ['Liste 30 types de partenaires','Template accord d\'apport','Email de présentation partenaires'],
        'tips'        => 'Commencez par les notaires et courtiers — ils voient des vendeurs potentiels avant même que ceux-ci pensent à vendre.',
    ],
    [
        'key'         => 'google_ads_local',
        'num'         => 5,
        'titre'       => 'Lancer Google Ads local (Search + LSA)',
        'desc'        => 'Google Ads Search sur des mots-clés d\'intention forte : "estimer maison [ville]", "vendre appartement [quartier]". Budget 10-20€/jour. Les annonces Local Service Ads (LSA) sont encore plus efficaces pour l\'immobilier.',
        'duree'       => '2h',
        'difficulte'  => 'Difficile',
        'module_lien' => ADMIN_URL . '/dashboard.php?page=strategy-module',
        'module_nom'  => 'Vue stratégie',
        'module_icon' => 'fa-google',
        'ressources'  => ['Guide Google Ads immobilier','Liste mots-clés locaux','Structure de campagne Search'],
        'tips'        => 'Commencez par les Local Service Ads (LSA) — plus simples et moins chers que le Search classique pour les débutants.',
    ],
];
...
try {
    $db   = getDB();
    $stmt = $db->prepare("SELECT step_key, status, note FROM ancre_progress WHERE instance_id=:iid AND user_id=:uid AND pilier='E'");
...
$coach_pilier = [
    'lettre'  => 'E',
    'mot'     => 'Expansion continue',
    'contexte'=> "Le pilier E — Expansion continue consiste à :
1. Lancer une campagne Facebook Ads vendeurs (35-65 ans, propriétaires, 5-10€/jour)
2. Installer le pixel Meta et configurer le retargeting (visiteurs page estimation)
3. Créer 3 audiences personnalisées : visiteurs, CRM importés, Lookalike 1%
4. Mettre en place un programme de partenaires locaux (notaires, courtiers, artisans)
5. Lancer Google Ads Search + Local Service Ads sur mots-clés d'intention forte

L'objectif est de scaler l'acquisition de leads en payant uniquement quand la croissance organique a atteint ses limites, ou en parallèle pour dominer le marché local.",
    'suggestions' => [
        'Créer ma première pub vendeurs',
        'Comment configurer le pixel Meta ?',
        'Trouver mes 10 partenaires locaux',
        'Budget pub recommandé pour débuter',
    ],
];
?>
...
.pilier-wrap{--pc:#8b5cf6;--pc-light:#f5f3ff;--pc-border:#ddd6fe;--pc-text:#4c1d95;--surface:var(--surface,#fff);--surface-2:var(--surface-2,#f9fafb);--border:var(--border,#e5e7eb);--radius:var(--radius-lg,12px);--shadow:var(--shadow-sm,0 1px 3px rgba(0,0,0,.08));--text:var(--text,#111827);--text-2:var(--text-2,#6b7280);--text-3:var(--text-3,#9ca3af);font-family:'DM Sans',sans-serif;max-width:900px;margin:0 auto}
...
.step-badge.difficile{background:#fee2e2;color:#991b1b}
...
.step-status-select{padding:5px 10px;background:var(--surface-2);border:1px solid var(--border);border-radius:20px;font-size:.7rem;font-weight:600;color:var(--text-2);cursor:pointer;outline:none;appearance:none;font-family:inherit}
...
.step-module-btn{grid-column:1 / -1;display:inline-flex;align-items:center;gap:8px;padding:10px 18px;background:var(--pc);color:#fff;border-radius:var(--radius);font-size:.78rem;font-weight:700;text-decoration:none;width:fit-content;transition:transform .15s}
.step-module-btn:hover{transform:translateX(3px);color:#fff}
.step-note-wrap{grid-column:1 / -1;margin-top:4px}
.step-note{width:100%;box-sizing:border-box;background:var(--surface-2);border:1px solid var(--border);border-radius:8px;padding:8px 12px;font-size:.78rem;color:var(--text);font-family:inherit;resize:vertical;min-height:60px;outline:none}
.step-note:focus{border-color:var(--pc)}
.step-note-save{margin-top:6px;padding:5px 14px;background:var(--surface-2);border:1px solid var(--border);border-radius:6px;font-size:.72rem;font-weight:600;color:var(--text-2);cursor:pointer}
...
.pilier-final-bravo h2{
    font-family:'Fraunces',Georgia,serif;
    font-size:1.6rem;font-weight:900;
    color:#fff;margin:0 0 10px;
}
.pilier-final-bravo p{
...
.bravo-btn:hover{transform:translateY(-2px);box-shadow:0 8px 24px rgba(201,145,59,.4);color:#fff}
...
<div class="pilier-wrap">
    <div class="pilier-breadcrumb anim">
        ...
        <span>/</span>
        <span style="color:var(--pc);font-weight:700">E — Expansion continue</span>
    </div>
    <div class="pilier-hero anim">
        <div class="pilier-hero-left">
            <div class="pilier-letter-badge">E</div>
            <h1 class="pilier-hero-title">Expansion continue</h1>
            <p class="pilier-hero-sub">Scalez votre acquisition via la publicité payante, le retargeting et les partenariats. Ce pilier démultiplie ce que vous avez construit dans A, N, C et R.</p>
        </div>
    ...
    <div class="pilier-nav anim">
        ...
        <a href="<?= ADMIN_URL ?>/dashboard.php?page=<?= $pg ?>" class="pilier-nav-item <?= $l==='E'?'current':'' ?>">
            <span class="pilier-nav-dot" style="<?= $l==='E'?'background:var(--pc)':'' ?>"></span>
            <strong><?= $l ?></strong> — <?= $m ?>
        </a>
        <?php endforeach; ?>
    </div>
    <div class="pilier-steps anim">
        <?php foreach ($steps as $step):
            ...
            $dc=match($step['difficulte']){'Facile'=>'easy','Difficile'=>'difficile','Continu'=>'continu',default=>'medium'};
        ?>
    ...
    <!-- Bloc final — Méthode ANCRE complète -->
    <div class="pilier-final-bravo anim">
        <h2>🎯 Vous maîtrisez la Méthode ANCRE</h2>
        <p>Vous avez parcouru les 5 piliers. La vraie puissance vient de leur synergie — revenez régulièrement affiner chaque pilier et suivre votre progression globale.</p>
        <a href="<?= ADMIN_URL ?>/dashboard.php?page=ancre" class="bravo-btn">
            <i class="fas fa-anchor"></i> Vue d'ensemble ANCRE
        </a>
    </div>

    <div class="pilier-footer-nav anim">
        <a href="<?= ADMIN_URL ?>/dashboard.php?page=ancre-r" class="pilier-back-btn">
            <i class="fas fa-arrow-left"></i> R — Relation
        </a>
        <a href="<?= ADMIN_URL ?>/dashboard.php?page=ancre" class="pilier-back-btn" style="border-color:var(--pc);color:var(--pc)">
            <i class="fas fa-anchor"></i> Retour à la méthode ANCRE
        </a>
    </div>
</div>
...
(function(){
    ...
    const PILIER='E',TOTAL=<?= $total_steps ?>;
