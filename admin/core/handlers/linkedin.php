<?php
/**
 * API Handler: linkedin
 * Called via: /admin/api/router.php?module=linkedin&action=...
 * Table: social_posts (filtered by platform='linkedin')
 */
...
switch ($action) {
    case 'list':
        try {
            ...
            $where = "WHERE platform = 'linkedin'"; $params = [];
    ...
    case 'get':
        try {
            ...
            $stmt = $pdo->prepare("SELECT * FROM social_posts WHERE id = ? AND platform = 'linkedin'");
            $stmt->execute([$id]);
    ...
    case 'create':
        try {
            $stmt = $pdo->prepare("INSERT INTO social_posts (platform, post_type, content, media_url, hashtags, scheduled_at, status, persona_id) VALUES ('linkedin', ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([
            ...
            echo json_encode(['success' => true, 'message' => 'Post LinkedIn cree', 'id' => $pdo->lastInsertId()]);
        } catch (PDOException $e) {
            echo json_encode(['success' => false, 'message' => $e->getMessage()]);
        }
        break;
    ...
    case 'update':
        try {
            ...
            $params[] = $id;
            $pdo->prepare("UPDATE social_posts SET " . implode(', ', $sets) . " WHERE id = ? AND platform = 'linkedin'")->execute($params);
    ...
    case 'delete':
        try {
            $id = (int)($input['id'] ?? 0);
            $pdo->prepare("DELETE FROM social_posts WHERE id = ? AND platform = 'linkedin'")->execute([$id]);
    ...
    case 'publish':
        try {
            $id = (int)($input['id'] ?? 0);
            $pdo->prepare("UPDATE social_posts SET status = 'published', published_at = NOW() WHERE id = ? AND platform = 'linkedin'")->execute([$id]);
    ...
    case 'stats':
        try {
            $stats = [
                ...
                'scheduled' => (int)$pdo->query("SELECT COUNT(*) FROM social_posts WHERE platform = 'linkedin' AND status = 'scheduled'")->fetchColumn(),
                'draft' => (int)$pdo->query("SELECT COUNT(*) FROM social_posts WHERE platform = 'linkedin' AND status = 'draft'")->fetchColumn(),
                'total_likes' => (int)$pdo->query("SELECT COALESCE(SUM(engagement_likes), 0) FROM social_posts WHERE platform = 'linkedin'")->fetchColumn(),
            ];
